...
def check_http_traffic(request):
  ...
  class connection_handler(urllib.request.HTTPHandler, urllib.request.HTTPSHandler):
    if settings.SCHEME == 'https':
...
def do_check(request):
  ...
  if not checks.get_header(request.headers, settings.HTTP_ACCEPT_HEADER):
    requests.Request('GET',settings.HTTP_ACCEPT_HEADER, settings.HTTP_ACCEPT_HEADER_VALUE)
