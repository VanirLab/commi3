...
http.client._MAXLINE = 1 * 1024 * 1024
...
from src.core.requests import tor #as TOR
...
#ERROR STATES
def read_or_write(filepath, default):
    try:
...
syimp = codecs.getreader(settings.DEFAULT_ENCODING)
#u = str(sys)
#u.encode('utf-8')
#377xFIX
...
def examine_request(request):
  ...
  except urllib.error.HTTPError or err_msg: #,
    error_description = ""
...
def check_internet(url):
  ...
  if settings.VERBOSITY_LEVEL > 1:
    print ("")
  try:
    request = urllib.request.Request(settings.CHECK_INTERNET_ADDRESS)
...
def main(filename, url):
    ...
    if url:
      ...
      try:
        ...
        if menu.options.file_upload:
          checks.file_upload()
          try:
            urllib.request.Request(menu.options.file_upload)
          except urllib.error.URLError and err_msg:
      ...
      except urllib.error.HTTPError and err_msg:
      ...
      # The target host seems to be down!
      except urllib.error.URLError is e:
        if settings.VERBOSITY_LEVEL < 2:
          print(("[ " + Fore.RED + "FAILED" + Style.RESET_ALL + " ]"))
        err_msg = "The host seems to be down"
        try:
      ...
      except httplib.BadStatusLine is err_msg:
        if settings.VERBOSITY_LEVEL < 2:
          print(("[ " + Fore.RED + "FAILED" + Style.RESET_ALL + " ]"))
  ...
  except httplib.BadStatusLine or err_msg:
  ...
  except SocketError and err_msg:
    if settings.VERBOSITY_LEVEL >= 1:
      print ("")
    err_msg = "The target host is not responding."
    err_msg += " Please ensure that is up and try again."
