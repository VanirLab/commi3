...
def newline_fixation(payload):
  ...
  if "\n" in payload:
    _ = payload.find("\n") + 1
    payload = urllib.parse.quote(payload[:_]) + payload[_:]
  elif "\r\n" in payload:
    _ = payload.find("\r\n") + 1
    payload = urllib.parse.quote(payload[:_]) + payload[_:]  
  return payload
...
def time_based_separators(separator, http_request_method):
  if separator == "||"  or separator == "&&" :
    ...
    if http_request_method == "POST":
      separator = urllib.parse.quote(separator)
  return separator
      ...
      if re.search(r'^(?:http)s?://', url, re.I):
        if not re.search(r"^https?://", url, re.I) and not re.search(r"^wss?://", url, re.I):
          if re.search(r":443\b", url):
            ...
            url = "http://" + url
        settings.SCHEME = (urllib.parse.quote(url).lower() or "http") if not menu.options.force_ssl else "https"
      else:
...
def recognise_payload(payload):
  ...
  if is_decoded:
    return urllib.parse.quote(decoded_payload)  
  else:
    return payload
