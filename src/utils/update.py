...
def updater():
  ...
  if menu.options.offline:  
    print ("[" + Fore.RED + " FAILED " + Style.RESET_ALL + "]")
    err_msg = "You cannot update commi3 via GitHub without access on the Internet."
...
"""
Check for new version of commi3
"""
def check_for_update():
  try:
    response = urllib.request.urlopen('https://raw.githubusercontent.com/VanirLab/commi3/master/src/utils/settings.py', timeout=1)
...
def check_unicorn_version(current_version):
  try:
    if len(current_version) != 0: 
      response = urllib.request.urlopen('https://raw.githubusercontent.com/trustedsec/unicorn/master/unicorn.py', timeout=1)
