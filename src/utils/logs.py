...
def update_payload(filename, counter, payload):
  output_file = open(filename, "a")
  if "\n" in payload:
    output_file.write("    (" +str(counter)+ ") Payload: " + re.sub("%20", " ", urllib.parse.unquote_plus(payload.replace("\n", "\\n"))) + "\n")
  else:
...
def log_traffic(header):
  ...
  output_file.read(session_handler) #Reads the sessions
  output_file.close()
